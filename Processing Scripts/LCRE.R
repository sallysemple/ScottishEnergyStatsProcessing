...
lcreedataset2020 <- read_excel("Data Sources/LCRE Survey/lcreedataset2020.xlsx", 
                               sheet = "LCRE by country")

lcreedataset2020 <- fill(lcreedataset2020,1, .direction ="down")

lcreedataset2020 <- as_tibble(lcreedataset2020)

lcrelist <- list()


min(as.numeric(lcreedataset2020[2,]), na.rm = TRUE)

for (year in min(as.numeric(lcreedataset2020[2,]), na.rm = TRUE):max(as.numeric(lcreedataset2020[2,]), na.rm = TRUE)){
  print(year)
  i<- year -  min(as.numeric(lcreedataset2020[2,]), na.rm = TRUE) + 1
  print(i)
  data <- lcreedataset2020[c(1,2,((i*4)-1):(((i*4)-1)+3))]
  data$Year <- year
  lcrelist[[i]] <- data
}
...
lcreedataset2020 <- fill(lcreedataset2020,1, .direction ="down")
lcreedataset2020 <- fill(lcreedataset2020,2, .direction ="down")
...
min(as.numeric(lcreedataset2020[4,]), na.rm = TRUE)

for (year in min(as.numeric(lcreedataset2020[4,]), na.rm = TRUE):max(as.numeric(lcreedataset2020[4,]), na.rm = TRUE)){
  print(year)
  i<- year -  min(as.numeric(lcreedataset2020[4,]), na.rm = TRUE) + 1
  print(i)
  data <- lcreedataset2020[c(1,2,3,((i*4)):(((i*4))+3))]
  data$Year <- year
  lcrelist[[i]] <- data
}
